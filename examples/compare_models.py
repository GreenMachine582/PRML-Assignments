...
def compareEstimators(dataset: Dataset, config: Config) -> None:
    ...
    models = {'GBR': ensemble.GradientBoostingRegressor(random_state=config.random_state),
              ...
              'MLPR': neural_network.MLPRegressor(max_iter=5000, random_state=config.random_state),
              'SVR': svm.SVR(),
    ...
    ml.estimator.plotPrediction(y_train, y_test, y_preds, ylabel=dataset.target, dataset_name=dataset.name,
...
def compareClassifiers(dataset: Dataset, config: Config) -> None:
    ...
    models = {'GBC': ensemble.GradientBoostingClassifier(random_state=config.random_state),
              ...
              'MLPC': neural_network.MLPClassifier(max_iter=1600, random_state=config.random_state),
              'SVC': svm.SVC(),
